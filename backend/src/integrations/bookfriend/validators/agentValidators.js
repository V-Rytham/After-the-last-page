...
export const validateStartPayload = (body) => {
  ...
  if (body?.chapter_progress != null) {
    const cp = Number.parseInt(String(body.chapter_progress), 10);
...
export const validateEndPayload = (body) => {
  if (!String(body?.session_id || '').trim()) throw new ValidationError('session_id is required');
};
